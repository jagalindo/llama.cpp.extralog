...
// Return (and lazily open) the weights.csv file handle.
// Must be called inside ggml_critical_section.
static FILE * billaud_weights_file(void) {
    if (!g_billaud_weights_file) {
        g_billaud_weights_file = fopen("weights.csv", "a");
        if (!g_billaud_weights_file) {
            perror("Failed to open weights.csv");
            exit(EXIT_FAILURE);
        }
        // Write CSV header only when the file is new/empty.
        fseek(g_billaud_weights_file, 0, SEEK_END);
        if (ftell(g_billaud_weights_file) == 0) {
            fprintf(g_billaud_weights_file,
                "op;name;ne0;ne1;ne2;"
                "min;max;mean;mean_sq;var;std;"
                "zero_ratio;pos_ratio;neg_ratio;"
                "lt_-1;-1_to_-0.5;-0.5_to_-0.1;-0.1_to_0;"
                "0_to_0.1;0.1_to_0.5;0.5_to_1;gt_1\n");
        }
    }
    return g_billaud_weights_file;
}

// Accumulate statistics for a single float value.
static void billaud_accum(double v,
    double * mn, double * mx, double * sum, double * sum_sq,
    double * zero, double * pos, double * neg, double h[8])
{
    if (v < *mn) *mn = v;
    if (v > *mx) *mx = v;
    *sum    += v;
...
// Compute stats over tensor elements and write one CSV row.
// In the binary path the dst output buffer is scanned; in the unary path
// src[p] rows are scanned directly.
void BILLAUD_print_float_array_weigth(FILE * f, struct ggml_tensor * dst,
        int size, int n, int p, bool binary_) {
    ...
    if (binary_) {
        ...
        if (ggml_is_contiguous(dst)) {
            for (int64_t i13 = 0; i13 < ne13; i13++) {
                for (int64_t i12 = 0; i12 < ne12; i12++) {
                    float * row = (float *)((char *)dst->data + i12*nb2 + i13*nb3);
                    for (int i = 0; i < size; i++) {
                        billaud_accum((double)row[i],
                            &mn, &mx, &sum, &sum_sq, &zero, &pos, &neg, h);
                        total++;
                    }
                }
            }
        ...
        for (int j = 0; j < n; j++) {
            float * row = (float *)((char *)dst->src[p]->data + j * dst->src[p]->nb[1]);
            for (int i = 0; i < size; i++) {
                billaud_accum((double)row[i],
                    &mn, &mx, &sum, &sum_sq, &zero, &pos, &neg, h);
                total++;
    ...
    double mean_sq = sum_sq / total;
    double var     = mean_sq - mean * mean;
    double std_dev = sqrt(var < 0.0 ? 0.0 : var);

    fprintf(f, "%.4f;%.4f;%.4f;%.4f;%.4f;%.4f;"
               "%.4f;%.4f;%.4f;"
               "%.4f;%.4f;%.4f;%.4f;%.4f;%.4f;%.4f;%.4f\n",
        mn, mx, mean, mean_sq, var, std_dev,
        zero/total, pos/total, neg/total,
        h[0]/total, h[1]/total, h[2]/total, h[3]/total,
        h[4]/total, h[5]/total, h[6]/total, h[7]/total);
}

// Log statistics for the source tensor whose name contains `name`.
// Serialised via ggml_critical_section to avoid concurrent file writes.
void BILLAUD_print_weight_f32(struct ggml_tensor * dst, const char * name, bool binary) {
    ggml_critical_section_start();

    FILE * f = billaud_weights_file();

    if (dst->src[0] && strstr(dst->src[0]->name, name)) {
        ...
        fprintf(f, "%s;%s;%lld;%lld;%lld;",
            ggml_op_name(dst->op), dst->src[0]->name,
            (long long)dst->src[0]->ne[0],
            (long long)dst->src[0]->ne[1],
            (long long)dst->src[0]->ne[2]);
        BILLAUD_print_float_array_weigth(f, dst, nc, n, 0, binary);
    } else if (dst->src[1] && strstr(dst->src[1]->name, name)) {
        const int n  = ggml_nrows(dst->src[1]);
        const int nc = dst->src[1]->ne[0];
        GGML_ASSERT(dst->src[1]->nb[0] == sizeof(float));
        fprintf(f, "%s;%s;%lld;%lld;%lld;",
            ggml_op_name(dst->op), dst->src[1]->name,
            (long long)dst->src[1]->ne[0],
            (long long)dst->src[1]->ne[1],
            (long long)dst->src[1]->ne[2]);
        BILLAUD_print_float_array_weigth(f, dst, nc, n, 1, binary);
    }
...
// Dispatch to weight-specific logging for each tracked weight pattern.
void BILLAUD_weight_repartition(struct ggml_tensor * dst) {
    if (!dst->src[0] || !dst->src[1]) return;

    const char * name1 = dst->src[0]->name;
    const char * name2 = dst->src[1]->name;

    static const struct { const char * pattern; bool binary; } weights[] = {
        { "attn_norm.weight",   false },
        { "attn_qkv.weight",    true  },
        { "attn_output.weight", true  },
        { "ffn_norm.weight",    false },
        { "ffn_up.weight",      true  },
        { "ffn_down.weight",    true  },
        { "output_norm.weight", false },
    };

    for (int i = 0; i < (int)(sizeof(weights)/sizeof(weights[0])); i++) {
        if (strstr(name1, weights[i].pattern) || strstr(name2, weights[i].pattern)) {
            BILLAUD_print_weight_f32(dst, weights[i].pattern, weights[i].binary);
        }
    }

    // output.weight: exclude attn_output variants
    if ((strstr(name1, "output.weight") && !strstr(name1, "attn_")) ||
        (strstr(name2, "output.weight") && !strstr(name2, "attn_"))) {
        BILLAUD_print_weight_f32(dst, "output.weight", true);
    }
}

// Alternate true/false on each call. Thread-safe via critical section.
bool toggle(void) {
